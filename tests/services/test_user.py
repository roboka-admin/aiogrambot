...
class FakeUserRepository:
    ...
    async def count_registered(self) -> int:
        return sum(user.registration_status == RegistrationStatus.REGISTERED for user in self.users.values())

    async def count_unregistered(self) -> int:
        return sum(user.registration_status == RegistrationStatus.UNREGISTERED for user in self.users.values())
    ...
    async def count_active_today(self, today_start) -> int:
        return sum(user.last_seen_at is not None and user.last_seen_at >= today_start for user in self.users.values())

    async def count_active_last_7_days(self, seven_days_ago) -> int:
        return sum(user.last_seen_at is not None and user.last_seen_at >= seven_days_ago for user in self.users.values())

    async def count_inactive_30_days(self, thirty_days_ago) -> int:
        return sum(user.last_seen_at is None or user.last_seen_at < thirty_days_ago for user in self.users.values())
