...
async def edit_profile_handler(message: Message) -> None:
    await message.answer("کدام مورد را می‌خواهید تغییر دهید؟", reply_markup=edit_profile_menu)
...
async def change_age_handler(message: Message, state: FSMContext) -> None:
...
async def cancel_edit_handler(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer("ویرایش لغو شد.", reply_markup=main_menu)


@router.message(EditProfileStates.waiting_name, F.text)
async def save_name_handler(message: Message, state: FSMContext, user: User, user_service: UserService) -> None:
    ...
    try:
        ...
        return
    await state.clear()
    await message.answer("نام شما با موفقیت تغییر کرد ✅", reply_markup=main_menu)
...
async def save_age_handler(message: Message, state: FSMContext, user: User, user_service: UserService) -> None:
    if message.text is None:
        return
    if not validate_age(message.text):
        ...
        return
    try:
        await user_service.update_age(user.telegram_id, int(message.text))
    except UserNotFoundError:
        await state.clear()
        return
    await state.clear()
    await message.answer("سن شما با موفقیت تغییر کرد ✅", reply_markup=main_menu)
