...
async def system_stats_handler(callback: CallbackQuery, system_service: SystemService) -> None:
    await _show_system_statistics(message=callback.message, system_service=system_service)
    await callback.answer()


@router.callback_query(AdminStatsRefreshCallback.filter(F.section == "system"))
...
def _format_memory(used_mb: int | None, total_mb: int | None, percent: float | None) -> str:
    if used_mb is None or total_mb is None or percent is None:
        return "—"
    return f"{percent:.1f}% ({used_mb / 1024:.1f} / {total_mb / 1024:.1f} GB)"


def _format_disk(used_gb: float | None, total_gb: float | None, percent: float | None) -> str:
