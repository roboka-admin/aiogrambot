...
@router.callback_query(AdminManagementCallback.filter(F.action == "back"))
async def admin_management_back_handler(
...
async def admin_view_handler(
    ...
    if admin.role is AdminRole.OWNER:
        ...
        titles = {permission.key: permission.title for permission in ADMIN_PERMISSION_REGISTRY}
        permission_text = "\n".join(
...
@router.callback_query(AdminManagementCallback.filter(F.action == "permissions"))
async def admin_permissions_handler(
    callback: CallbackQuery,
    callback_data: AdminManagementCallback,
    admin_service: AdminService,
) -> None:
    admin = await admin_service.get_admin(callback_data.telegram_id)
    if admin is None or admin.role is AdminRole.OWNER:
        await callback.answer("امکان مدیریت دسترسی این ادمین وجود ندارد.", show_alert=True)
        return
...
@router.callback_query(AdminPermissionCallback.filter())
async def admin_permission_update_handler(
    callback: CallbackQuery,
    callback_data: AdminPermissionCallback,
    admin_service: AdminService,
) -> None:
    current = await admin_service.get_permissions(callback_data.telegram_id)
    if callback_data.permission_key in current:
        current.remove(callback_data.permission_key)
    else:
        current.add(callback_data.permission_key)

    try:
        changed = await admin_service.set_managed_admin_permissions(
            actor_telegram_id=callback.from_user.id,
            telegram_id=callback_data.telegram_id,
            permission_keys=current,
        )
    ...
    if not changed:
        await callback.answer("امکان تغییر دسترسی وجود ندارد.", show_alert=True)
        return
...
async def admin_deactivate_handler(
    ...
    await callback.answer(
        "ادمین غیرفعال شد." if changed else "امکان انجام این کار وجود ندارد.",
...
async def admin_activate_handler(
    ...
    await callback.answer(
        "ادمین فعال شد." if changed else "امکان انجام این کار وجود ندارد.",
        show_alert=not changed,
    )
    if changed:
        admin = await admin_service.get_admin(callback_data.telegram_id)
...
async def admin_delete_handler(
    ...
    await callback.answer(
        "ادمین حذف شد." if removed else "امکان انجام این کار وجود ندارد.",
