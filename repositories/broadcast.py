...
class BroadcastRepository(IBroadcastRepository):
    ...
    async def get_latest(self) -> BroadcastRecord | None:
        result = await self._session.execute(
            select(BroadcastRecordRecord).order_by(BroadcastRecordRecord.id.desc()).limit(1)
        )
        db_record = result.scalar_one_or_none()
        return None if db_record is None else self._to_domain(db_record)

    async def count_total(self) -> int:
        result = await self._session.execute(select(func.count()).select_from(BroadcastRecordRecord))
        return result.scalar_one()

    async def count_today(self, today_start: datetime) -> int:
        result = await self._session.execute(
            select(func.count()).select_from(BroadcastRecordRecord).where(
                BroadcastRecordRecord.created_at.is_not(None),
                BroadcastRecordRecord.created_at >= today_start,
            )
        )
        return result.scalar_one()
    ...
    async def count_last_7_days(self, seven_days_ago: datetime) -> int:
        result = await self._session.execute(
            select(func.count()).select_from(BroadcastRecordRecord).where(
                BroadcastRecordRecord.created_at.is_not(None),
                BroadcastRecordRecord.created_at >= seven_days_ago,
            )
        )
        return result.scalar_one()
    ...
    async def count_last_30_days(self, thirty_days_ago: datetime) -> int:
        result = await self._session.execute(
            select(func.count()).select_from(BroadcastRecordRecord).where(
                BroadcastRecordRecord.created_at.is_not(None),
                BroadcastRecordRecord.created_at >= thirty_days_ago,
            )
        )
        return result.scalar_one()
