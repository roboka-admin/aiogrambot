...
async def start_registration(callback: CallbackQuery, state: FSMContext, user: User) -> None:
    if user.registration_status == RegistrationStatus.REGISTERED:
        await callback.answer("شما قبلاً ثبت نام کرده‌اید.", show_alert=True)
        return
...
async def process_age(message: Message, state: FSMContext, register_service: RegisterService) -> None:
